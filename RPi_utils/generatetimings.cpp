...
int main(int argc, char *argv[]) {
    ...
    unsigned int * transmitarray = new unsigned int[32*2 + 2 +1];
    transmitarray[0] = 0;
    mySwitch.transmittimings = transmitarray;
    mySwitch.send(code, 24);
    
    unsigned int currenttiming = 0;
    while( mySwitch.transmittimings[currenttiming] && currenttiming < 32*2 + 2 +1 ) {
      printf("%i, ", mySwitch.transmittimings[currenttiming] );
      currenttiming++;
    }
    printf("\n");
